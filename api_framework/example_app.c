...
void home_page(Request *req, Response *res)
{
    const char *html =
        ...
        "    \n"
        "    <div id='time-display'>Loading server time...</div>\n"
        "    \n"
        ...
        "    <div id='api-result' style='margin-top: 20px; padding: 10px; background: #f5f5f5; border-radius: 5px; display: none;'>\n"
        "        <h3>API Response:</h3>\n"
        ...
        "                .then(r => r.json())\n"
        "                .then(data => {\n"
        "                    document.getElementById('time-display').textContent = 'Server Time: ' + data.time;\n"
